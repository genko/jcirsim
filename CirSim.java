...
public class CirSim extends JFrame implements ComponentListener,
	...
	public void init() {
		...
		m
				.add(conventionCheckItem = getCheckItem("Conventional Current Motion"));
		...
		passMenu
				.add(getClassCheckItem("Add Transmission Line", "TransLineElm"));
		...
		if (stopMessage != null) {
			...
			if (mouseElm != null) {
				if (mousePost == -1)
					...
					info[0] = "V = "
							+ CircuitElm.getUnitText(mouseElm
									.getPostVoltage(mousePost), "V");
				/*
	...
	String getHint() {
		...
		if (hintType == HINT_LC) {
			...
			return "res.f = "
					+ CircuitElm.getUnitText(1 / (2 * pi * Math
							.sqrt(ie.inductance * ce.capacitance)), "Hz");
		}
		if (hintType == HINT_RC) {
	...
	void analyzeCircuit() {
		...
		for (i = 0; i != elmList.size(); i++) {
			...
			if (ce instanceof InductorElm) {
				FindPathInfo fpi = new FindPathInfo(FindPathInfo.INDUCT, ce, ce
						.getNode(1));
			...
			if (ce instanceof CapacitorElm) {
				FindPathInfo fpi = new FindPathInfo(FindPathInfo.SHORT, ce, ce
						.getNode(1));
				if (fpi.findPath(ce.getNode(0))) {
					System.out.println(ce + " shorted");
					ce.reset();
				} else {
					fpi = new FindPathInfo(FindPathInfo.CAP_V, ce, ce
							.getNode(1));
					if (fpi.findPath(ce.getNode(0))) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (ac.indexOf("setup ") == 0) {
			pushUndo();
			readSetupFile(ac.substring(6), ((MenuItem) e.getSource())
					.getLabel());
		}
	}
	...
	void doMainMenuChecks(Menu m) {
		...
		for (i = 0; i != m.getItemCount(); i++) {
			...
			if (mc instanceof CheckboxMenuItem) {
				...
				cmi
						.setState(mouseModeStr
								.compareTo(cmi.getActionCommand()) == 0);
			}
		}
	}
	...
	void pushUndo() {
		...
		if (undoStack.size() > 0
				&& s.compareTo((String) (undoStack.lastElement())) == 0)
			return;
		undoStack.add(s);
		enableUndoRedo();
	}
